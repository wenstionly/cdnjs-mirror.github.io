...
function scrollToPosition(position) {
    ...
    if ('selector' in position) {
        ...
        if (!el) {
                ...
                vue.warn(`Couldn't find element with selector "${position.selector}"`);
            return;
        }
...
function createRouterMatcher(routes, globalOptions) {
    ...
    function resolve(location, currentLocation) {
        ...
        if ('name' in location && location.name) {
            ...
            path = location.path;
            if ( path[0] !== '/') {
                vue.warn(`The Matcher cannot resolve relative paths but received "${path}". Unless you directly called \`matcher.resolve("${path}")\`, this is probably a bug in vue-router. Please open an issue at https://new-issue.vuejs.org/?repo=vuejs/vue-router-next.`);
            }
...
function createRouter(options) {
    ...
    function resolve(rawLocation, currentLocation) {
        ...
        matcher.resolve('params' in rawLocation
    ...
    function finalizeNavigation(toLocation, from, isPush, replace, data) {
        ...
        if (isBrowser) {
            const savedScroll = getSavedScrollPosition(getScrollKey(toLocation.fullPath, 0));
            handleScroll(toLocation, from, savedScroll || ((isFirstNavigation || !isPush) && state && state.scroll)).catch(err => {
                triggerError(err);
            });
        }
        markAsReady();
