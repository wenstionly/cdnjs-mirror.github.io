/*!
  ...
  * @license MIT
  */
import { warn, markRaw, inject, computed, unref, defineComponent, reactive, h, provide, ref, getCurrentInstance, nextTick } from 'vue';
...
function getScrollKey(path, distance) {
    const position = isBrowser && history.state ? history.state.position - distance : -1;
    return position + path;
}
function saveScrollOnLeave(key) {
...
const START_LOCATION_NORMALIZED = markRaw({
    path: '/',
    name: undefined,
    params: {},
    query: {},
...
function useLink(props) {
    ...
    const activeRecordIndex = computed(() => {
        ...
        if (!currentMatched)
            return -1;
        return currentRoute.matched.findIndex(isSameRouteRecord.bind(null, currentMatched));
    });
...
function createRouter({ history, routes, scrollBehavior, parseQuery: parseQuery$1 = parseQuery, stringifyQuery: stringifyQuery$1 = stringifyQuery, }) {
    ...
    async function pushWithRedirect(to, redirectedFrom) {
        ...
        try {
            ...
            if (pendingLocation !== toLocation) {
                failure = createRouterError(3 /* NAVIGATION_CANCELLED */, {
                    from,
                    to: toLocation,
                });
            }
            ...
            else if (error.type === 1 /* NAVIGATION_GUARD_REDIRECT */) {
    ...
    function finalizeNavigation(toLocation, from, isPush, replace, data) {
        ...
        handleScroll(toLocation, from, savedScroll || (state && state.scroll)).catch(err => triggerError(err));
        markAsReady();
    }
     ...
     */
    function triggerError(error, shouldThrow = false) {
        markAsReady(error);
...
export { Link, NavigationFailureType, START_LOCATION_NORMALIZED as START_LOCATION, View, createMemoryHistory, createRouter, createWebHashHistory, createWebHistory, onBeforeRouteLeave, parseQuery, stringifyQuery, useLink, useRoute, useRouter };
