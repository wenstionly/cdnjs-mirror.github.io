...
function () {
  function Bot(_ref) {
    ...
    game._classCallCheck(this, Bot);

    game._defineProperty(this, "enumerate", function (G, ctx, playerID) {
      ...
      return actions.map(function (a) {
        ...
        if (a.move !== undefined) {
          return game.makeMove(a.move, a.args, playerID);
        }

        if (a.event !== undefined) {
          return game.gameEvent(a.event, a.args, playerID);
        }
      });
    });
...
function (_Bot) {
  ...
  function MCTSBot(_ref) {
    ...
    game._classCallCheck(this, MCTSBot);

    _this = game._possibleConstructorReturn(this, game._getPrototypeOf(MCTSBot).call(this, {
      enumerate: enumerate,
      seed: seed
    }));
    ...
    _this.reducer = game.CreateGameReducer({
      game: game$1
    });
  ...
  game._createClass(MCTSBot, [{
    ...
    value: function playout(node) {
      ...
      for (var i = 0; i < playoutDepth && state.ctx.gameover === undefined; i++) {
        ...
        if (game._typeof(_ret) === "object") return _ret.v;
      }
...
/*#__PURE__*/
function (_Bot) {
  game._inherits(RandomBot, _Bot);

  function RandomBot() {
    game._classCallCheck(this, RandomBot);

    return game._possibleConstructorReturn(this, game._getPrototypeOf(RandomBot).apply(this, arguments));
  }

  game._createClass(RandomBot, [{
    key: "play",
...
async function Simulate(_ref2) {
  var game$1 = _ref2.game,
      bots = _ref2.bots,
      state = _ref2.state,
      depth = _ref2.depth;
  if (depth === undefined) depth = 10000;
  var reducer = game.CreateGameReducer({
    game: game$1,
