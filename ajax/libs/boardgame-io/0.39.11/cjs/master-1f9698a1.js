 ...
 */
class Master {
    constructor(game$1, storageAPI, transportAPI, auth) {
    ...
    async onUpdate(credAction, stateID, gameID, playerID) {
        ...
        state = result.state;
        if (state === undefined) {
            game.error(`game not found, gameID=[${key}]`);
            return { error: 'game not found' };
        }
        if (state.ctx.gameover !== undefined) {
            game.error(`game over - gameID=[${key}]`);
            return;
        }
        const reducer = game.CreateGameReducer({
            game: this.game,
        });
        ...
        if (action.type == game.UNDO || action.type == game.REDO) {
            if (state.ctx.currentPlayer !== playerID ||
                ...
                game.error(`playerID=[${playerID}] cannot undo / redo right now`);
                return;
            }
        }
        // Check whether the player is active.
        if (!this.game.flow.isPlayerActive(state.G, state.ctx, playerID)) {
            game.error(`player not active - playerID=[${playerID}]`);
            return;
        }
        // Check whether the player is allowed to make the move.
        if (action.type == game.MAKE_MOVE &&
            !this.game.flow.getMove(state.ctx, action.payload.type, playerID)) {
            game.error(`move not processed - canPlayerMakeMove=false, playerID=[${playerID}]`);
            return;
        }
        if (state._stateID !== stateID) {
            game.error(`invalid stateID, was=[${stateID}], expected=[${state._stateID}]`);
            return;
        }
