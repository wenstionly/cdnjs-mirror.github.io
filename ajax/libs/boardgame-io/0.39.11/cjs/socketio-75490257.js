...
var master = require('./master-1f9698a1.js');
...
var Transport = function Transport(_ref) {
  ...
  game._classCallCheck(this, Transport);
...
function LocalMaster(_ref) {
  var game$1 = _ref.game,
      bots = _ref.bots;
  var clientCallbacks = {};
  var initializedBots = {};

  if (game$1 && game$1.ai && bots) {
    for (var playerID in bots) {
      var bot = bots[playerID];
      initializedBots[playerID] = new bot({
        game: game$1,
        enumerate: game$1.ai.enumerate,
        seed: game$1.seed
      });
    }
  }
  ...
  var send = function send(_ref2) {
    ...
    if (callback !== undefined) {
      callback.apply(null, [type].concat(game._toConsumableArray(args)));
    }
  };
  ...
  var master$1 = new master.Master(game$1, new InMemory(), {
    send: send,
    sendAll: sendAll
  }, false);
...
/*#__PURE__*/
function (_Transport) {
  game._inherits(LocalTransport, _Transport);
  ...
  function LocalTransport(_ref4) {
    ...
    game._classCallCheck(this, LocalTransport);

    _this = game._possibleConstructorReturn(this, game._getPrototypeOf(LocalTransport).call(this, {
      store: store,
      gameName: gameName,
      playerID: playerID,
      gameID: gameID,
...
function Local(opts) {
  return function (transportOpts) {
    ...
    return new LocalTransport(game._objectSpread2({
      master: master
    }, transportOpts));
  };
}
...
/*#__PURE__*/
function (_Transport) {
  game._inherits(SocketIOTransport, _Transport);
  ...
  function SocketIOTransport() {
    ...
    game._classCallCheck(this, SocketIOTransport);

    _this = game._possibleConstructorReturn(this, game._getPrototypeOf(SocketIOTransport).call(this, {
      store: store,
      gameName: gameName,
      playerID: playerID,
      gameID: gameID,
  ...
  game._createClass(SocketIOTransport, [{
    key: "onAction",
    value: function onAction(state, action) {
...
function SocketIO() {
  ...
  return function (transportOpts) {
    return new SocketIOTransport(game._objectSpread2({
      server: server,
