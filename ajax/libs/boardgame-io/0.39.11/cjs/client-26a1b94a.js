...
var Debug = require('./Debug-a4c62056.js');
var initialize = require('./initialize-58dc4481.js');
...
function createDispatchers(storeActionType, innerActionNames, store, playerID, credentials, multiplayer) {
  return innerActionNames.reduce(function (dispatchers, name) {
    dispatchers[name] = function () {
      ...
      store.dispatch(game.ActionCreators[storeActionType](name, args, assumedPlayerID, credentials));
    };
...
function () {
  function _ClientImpl(_ref) {
    ...
    game._classCallCheck(this, _ClientImpl);

    this.game = game.ProcessGameConfig(game$1);
    ...
    this.subscribers = {};
    this._running = false;
    this.reducer = game.CreateGameReducer({
      game: this.game,
    ...
    this.reset = function () {
      _this.store.dispatch(game.reset(_this.initialState));
    };

    this.undo = function () {
      _this.store.dispatch(game.undo(playerID, credentials));
    };

    this.redo = function () {
      _this.store.dispatch(game.redo(playerID, credentials));
    };
    ...
    var LogMiddleware = function LogMiddleware(store) {
      return function (next) {
        return function (action) {
          ...
          switch (action.type) {
            case game.MAKE_MOVE:
            case game.GAME_EVENT:
              {
                var deltalog = state.deltalog;
                _this.log = [].concat(game._toConsumableArray(_this.log), game._toConsumableArray(deltalog));
                break;
              }
            ...
            case game.UPDATE:
              {
                ...
                _this.log = [].concat(game._toConsumableArray(_this.log), game._toConsumableArray(_deltalog));
                break;
              }
  ...
  game._createClass(_ClientImpl, [{
    ...
    value: function getState() {
      ...
      var ret = game._objectSpread2({}, state, {
        isActive: isActive,
        G: G,
        log: this.log
      });
