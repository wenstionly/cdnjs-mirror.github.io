...
var MaterializeDirective = (function () {
    ...
    MaterializeDirective.prototype.performElementUpdates = function () {
        ...
        if (this.isTimePicker()) {
            ...
            setTimeout(function () {
                if (_this.ngModel) {
                    picker_2.set('select', _this.ngModel);
                }
                else {
                    picker_2.set('select', jqueryPickerElement_2.val());
                }
                jqueryPickerElement_2.on('change', function (e) { return nativeElement_4.dispatchEvent(new Event('input')); });
            });
        }
        if (this.isChips()) {
