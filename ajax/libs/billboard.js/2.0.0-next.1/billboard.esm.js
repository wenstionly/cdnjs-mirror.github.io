...
var data$1 = {
    ...
    isGrouped: function (id) {
        return this.config.data_groups
            .map(function (v) { return v.indexOf(id) >= 0; })[0];
    },
    getXKey: function (id) {
        var $$ = this;
        var config = $$.config;
 ...
 */
var scale = {
    getScale: function (min, max, forTimeseries) {
        return (forTimeseries ?
            scaleTime() : scaleLinear()).range([min, max]);
    },
    /**
    ...
    getXScale: function (min, max, domain, offset) {
        var $$ = this;
        var scale = $$.scale.zoom || $$.getScale(min, max, $$.axis.isTimeSeries());
    ...
    getYScale: function (min, max, domain) {
        var $$ = this;
        var scale = $$.getScale(min, max, $$.axis.isTimeSeriesY());
...
var shape = {
    ...
    getShapeYMin: function (id) {
        var $$ = this;
        var yMin = $$.scale[$$.axis.getId(id)].domain()[0];
...
var text = {
    ...
    updateText: function (durationForExit) {
        ...
        $el.text = $el.main.selectAll("." + CLASS.texts).selectAll("." + CLASS.text)
            .data(function (d) { return (_this.isRadarType(d) ? d.values : dataFn(d)); });
        $el.text.exit()
            .transition()
...
var type = {
    ...
    isTypeOf: function (d, type) {
        ...
        var dataType = this.config.data_types[id];
        return isArray(type) ?
...
var ChartInternal = /** @class */ (function () {
    ...
    ChartInternal.prototype.initParams = function () {
        ...
        if (state.hasAxis) {
            ...
            var isDragZoom_1 = $$.config.zoom_enabled && $$.config.zoom_enabled.type === "drag";
...
var AxisRendererHelper = /** @class */ (function () {
    ...
    AxisRendererHelper.prototype.generateTicks = function (scale, isYAxes) {
        ...
        if (isYAxes && tickStepSize) {
            ...
            ticks = scale.ticks.apply(scale, (this.config.tickArguments || [])).map(function (v) { return (
...
var eventrect = {
    ...
    redrawEventRect: function () {
        ...
        var zoomEnabled = config.zoom_enabled;
        var eventRects = $$.$el.main.select("." + CLASS.eventRects)
            .style("cursor", zoomEnabled && zoomEnabled.type !== "drag" ? (config.axis_rotated ? "ns-resize" : "ew-resize") : null)
...
var clip = {
    ...
    getAxisClipX: function (forHorizontal) {
        var margin = this.state.margin;
        // axis line width + padding for left
        var left = Math.max(30, margin.left);
        return forHorizontal ? -(1 + left) : -(left - 1);
    },
    getAxisClipY: function (forHorizontal) {
        var margin = this.state.margin;
    ...
    getAxisClipWidth: function (forHorizontal) {
        var $$ = this;
        var _a = $$.state, margin = _a.margin, width = _a.width;
        var left = Math.max(30, margin.left);
        var right = Math.max(30, margin.right);
        // width + axis line width + padding for left/right
        return forHorizontal ?
            width + 2 + left + right : margin.left + 20;
    },
    getAxisClipHeight: function (forHorizontal) {
        var _a = this.state, margin = _a.margin, height = _a.height;
...
var shapeArc = {
    ...
    textForArcLabel: function (selection) {
        ...
        if ($$.shouldShowArcLabel()) {
            selection.each(function (d) {
                ...
                var isUnderThreshold = !(!hasGauge && !$$.meetsArcLabelThreshold(ratio));
                if (isUnderThreshold) {
    ...
    meetsArcLabelThreshold: function (ratio) {
        var $$ = this;
        var config = $$.config;
        var threshold = $$.hasType("donut") ? config.donut_label_threshold : config.pie_label_threshold;
    ...
    updateTargetsForArc: function (targets) {
        ...
        var classFocus = $$.classFocus.bind($$);
        var mainPieUpdate = main.select("." + CLASS.chartArcs)
...
var shapeLine = {
    ...
    updateTargetsForLine: function (targets) {
        ...
        var mainLineUpdate = $el.main.select("." + CLASS.chartLines)
            .selectAll("." + CLASS.chartLine)
            .data(targets)
...
var shapePoint = {
    ...
    updateTargetForCircle: function (t) {
        var $$ = this;
        ...
        var targets = (t || data.targets)
            .filter(function (v) { return (!$$.isBarType(v) && (!$$.isLineType(v) || $$.shouldDrawPointsForLine(v)) && $$.labelishData(v)); });
    ...
    generateGetLinePoints: function (lineIndices, isSubValue) {
        var $$ = this;
        var config = $$.config;
        var isSub = !!isSubValue;
