...
var initializeTabs = function (tabs) {
  ...
  var optionSelected = function (e) {
    ...
    console.log('option Selected ' + $option.attr('value'))
    $target = $(this).parents('.tabbable').eq(0).find('> .tab-content > [data-idx=' + targetIdx + ']');

    // If the user explicitely chose one alternative, we must tell that this alternative has not a default
    // value anymore
...
jsonform.elementTypes = {
  ...
  'selectfieldset': {
    ...
    'onChange': function(evt, node) {
      ...
      const value = evt.target.options.selectedIndex
      _.each(node.children, function (child, idx) {
        if (idx === value) {
          console.log(child)
          child.defaultValue = false
          // child.value = "COUCOU"
        }
      });
    },
    ...
    'onBeforeRender': function (data, node) {
      ...
      console.log('onBeforeRender', children)
      var activeChild = null;
      if (data.value) {
        console.log('data.Value', activeChild)
        activeChild = _.find(children, function (child) {
          return (child.value === node.value);
        });
      }
      if (!activeChild) {
        console.log('non default', activeChild)
        activeChild = _.find(children, function (child) {
          console.log(child.node, child.node.hasNonDefaultValue())
          return child.node.hasNonDefaultValue();
        });
      }
      if (!activeChild) {
        console.log('first one', activeChild)
...
formNode.prototype.hasNonDefaultValue = function () {
  console.log('   comparing ' + this.value + ' to ' + this.defaultValue)
...
formNode.prototype.computeInitialValues = function (values, ignoreDefaultValues) {
  ...
  if (this.view && this.view.inputfield && this.schemaElement) {
    ...
    console.log('here first ' + this.value + ' ' + this.defaultValue)
    if (values) {
      console.log('   here second ' + this.value + ' ' + this.defaultValue)
      ...
      console.log('   here third ' + this.value + ' ' + this.defaultValue)
      console.log(this)
      // Special case for selects in a selectfieldset in an array (drag/dropping an item):
      // The default values are automatically selected in the case of a select
      // field, and thus, act as if they were selected by the user. Prevent that.
      if (typeof this.value !== 'undefined' && (typeof this.defaultValue === 'undefined' || this.defaultValue === false) && 'enum' in this.schemaElement) {
        //this.defaultValue = this.value === this.schemaElement['enum'][0]
        console.log('IT IS THE DEFAULT: ' + this.defaultValue)
      }
    }
    ...
    console.log('FINALLY ' + this.value + ' ' + this.defaultValue + ' - '  + JSON.stringify(this.schemaElement))
  }
  else if (this.view && this.view.array) {
