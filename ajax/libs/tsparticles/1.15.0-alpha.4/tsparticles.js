...
class AbsorbersPlugin_AbsorbersPlugin {
  ...
  needsPlugin(container) {
    ...
    if (absorbers instanceof Array) {
      ...
      loadAbsorbers = true;
    } else if (Utils_Utils.isInArray(ClickMode.absorber, options.interactivity.events.onClick.mode)) {
      loadAbsorbers = true;
    }
...
class Emitters_Emitters {
  ...
  handleClickMode(mode) {
    ...
    if (mode === ClickMode.emitter) {
      ...
      const emitter = new Emitter_Emitter(this, Utils_Utils.deepExtend({}, emitterOptions), ePosition);
      this.addEmitter(emitter);
    }
  }
  ...
  drawLinkTriangle(p1, link1, link2) {
    ...
    if (!colorTriangle) {
      if (container.particles.linksColor === Constants.randomColorValue) {
        ...
        colorTriangle = container.particles.linksColor;
      }
    }
  ...
  drawLinkLine(p1, link) {
    ...
    if (!colorLine) {
      if (container.particles.linksColor === Constants.randomColorValue) {
        ...
        colorLine = container.particles.linksColor;
      }
    }
...
class Particle_Particle {
  constructor(container, position, overrideOptions) {
    ...
    if (overrideOptions) {
      const shape = (_a = overrideOptions.shape) !== null && _a !== void 0 ? _a : particlesOptions.shape;
        ...
        if (!container.particles.linksColor) {
          ...
          if (color === Constants.randomColorValue) {
            if (p1.particlesOptions.links.consent) {
              container.particles.linksColor = ColorUtils_ColorUtils.colorToRgb({
                value: color
              });
...
class index_slim_Main {
  constructor() {
    ...
    Plugins.addShapeDrawer(ShapeType.star, new StarDrawer_StarDrawer());
