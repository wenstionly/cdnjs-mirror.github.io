...
class Repulser_Repulser {
    ...
    if (mouseMoveStatus && hoverEnabled && Utils_Utils.isInArray(HoverMode.repulse, hoverMode)) {
      ...
      if (divs instanceof Array) {
        for (const div of divs) {
          ...
          if (divEnabled && Utils_Utils.isInArray(DivMode.repulse, divMode)) {
  ...
  static singleDivRepulse(container, id, div) {
    ...
    const area = div.type === DivType.circle ? new Circle_Circle(pos.x, pos.y, repulseRadius) : new Rectangle_Rectangle(elem.offsetLeft, elem.offsetTop, elem.offsetWidth, elem.offsetHeight);
  ...
  static hoverRepulse(container) {
    ...
    this.processRepulse(container, mousePos, repulseRadius, new Circle_Circle(mousePos.x, mousePos.y, repulseRadius));
  }
    ...
    if (hoverEnabled && Utils_Utils.isInArray(HoverMode.bubble, hoverMode)) {
      ...
      if (divs instanceof Array) {
        for (const div of divs) {
          ...
          if (divEnabled && Utils_Utils.isInArray(DivMode.bubble, divMode)) {
...
class Core_Particles_Particles {
  ...
  update(delta) {
    ...
    if (divs instanceof Array) {
      ...
      divRepulse = modes.find(t => Utils_Utils.isInArray(DivMode.repulse, t)) !== undefined;
      divBubble = modes.find(t => Utils_Utils.isInArray(DivMode.bubble, t)) !== undefined;
    } else {
      divEnabled = divs.enable;
      divRepulse = Utils_Utils.isInArray(DivMode.repulse, divs.mode);
      divBubble = Utils_Utils.isInArray(DivMode.bubble, divs.mode);
    }
    ...
    if (divEnabled || events.onHover.enable && mouse.position || events.onClick.enable && mouse.clickPosition) {
      ...
      if (Utils_Utils.isInArray(HoverMode.repulse, hoverMode) || Utils_Utils.isInArray(ClickMode.repulse, clickMode) || divRepulse) {
        Repulser_Repulser.repulse(container, delta);
      }

      if (Utils_Utils.isInArray(HoverMode.bubble, hoverMode) || Utils_Utils.isInArray(ClickMode.bubble, clickMode) || divBubble) {
...
class Container_Container {
  ...
  draw() {
    this.drawAnimationFrame = Container_Container.requestFrame(t => {
      var _a;
