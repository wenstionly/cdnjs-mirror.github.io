...
class Particle_Particle {
  constructor(container, position, emitter) {
    ...
    if (this.shape === ShapeType.image || this.shape === ShapeType.images) {
      ...
      const drawer = container.drawers[this.shape];
...
// CONCATENATED MODULE: ./dist/Core/Emitter.js
...
class Infecter_Infecter {
  static infect(p1, container) {
    ...
    const infectedStage = (_a = infectionStage.infectedStage) !== null && _a !== void 0 ? _a : p1.infectionStage;
...
class Absorbers_Absorbers {
  ...
  init() {
    ...
    if (options.absorbers instanceof Array) {
      for (const absorberOptions of options.absorbers) {
        const absorber = new Plugins_Absorber_Absorber(container, absorberOptions);
  ...
  handleClickMode(mode) {
    ...
    if (mode === ClickMode.absorber) {
      ...
      const absorber = new Plugins_Absorber_Absorber(container, absorbersOptions, bhPosition);
      this.absorbers.push(absorber);
    }
  }
...
class dist_Main {
  constructor() {
    ...
    CanvasUtils_CanvasUtils.addShapeDrawer(ShapeType.image, new ImageDrawer_ImageDrawer());
    CanvasUtils_CanvasUtils.addShapeDrawer(ShapeType.images, new ImageDrawer_ImageDrawer());
  }
