...
class Utils_Utils {
  ...
  static get animate() {
    const animate = this.isSsr() ? callback => setTimeout(callback) : (callback => window.requestAnimationFrame(callback)) || (callback => window.webkitRequestAnimationFrame(callback)) || (callback => window.mozRequestAnimationFrame(callback)) || (callback => window.oRequestAnimationFrame(callback)) || (callback => window.msRequestAnimationFrame(callback)) || (callback => window.setTimeout(callback));
    return animate;
  }

  static get cancelAnimation() {
    const cancelAnimation = this.isSsr() ? handle => clearTimeout(handle) : (handle => window.cancelAnimationFrame(handle)) || (handle => window.webkitCancelRequestAnimationFrame(handle)) || (handle => window.mozCancelRequestAnimationFrame(handle)) || (handle => window.oCancelRequestAnimationFrame(handle)) || (handle => window.msCancelRequestAnimationFrame(handle)) || (handle => window.clearTimeout(handle));
