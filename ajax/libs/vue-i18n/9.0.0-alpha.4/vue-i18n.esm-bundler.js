/*!
  ...
  */
import { defineComponent, h, Fragment, ref, getCurrentInstance, computed, createTextVNode, provide, inject } from 'vue';
...
function createTokenizer(source) {
    ...
    const peekNewLines = (scnr) => {
        while (scnr.currentPeek() === CHAR_LF) {
            scnr.peek();
        }
    };
    const skipNewLines = (scnr) => {
        peekNewLines(scnr);
        scnr.skipToPeek();
    };
    ...
    const readLinkedRefer = (scnr, context) => {
    ...
    const readTokenInPlaceholder = (scnr, context) => {
        ...
        switch (ch) {
            ...
            default:
                ...
                else if ((validNamedIdentifier = isNamedIdentifierStart(scnr, context))) {
                    token = getToken(context, 5 /* Named */, readNamedIdentifier(scnr));
                    skipSpaces(scnr);
                }
                else if ((validListIdentifier = isListIdentifierStart(scnr, context))) {
                    token = getToken(context, 6 /* List */, readListIdentifier(scnr));
                    skipSpaces(scnr);
                }
                else if (!validNamedIdentifier && !validListIdentifier) {
                    token = getToken(context, 14 /* InvalidPlace */, readInvalidIdentifier(scnr));
                    skipSpaces(scnr);
                }
                break;
        }
        ...
        switch (ch) {
            ...
            default:
                if (isPluralStart(scnr)) {
                    ...
                    token = getToken(context, 11 /* LinkedModifier */, readLinkedModifier(scnr));
                    skipNewLines(scnr);
                }
                    ...
                    if (ch === "{" /* BraceLeft */) {
                        ...
                        token = getToken(context, 10 /* LinkedKey */, readLinkedRefer(scnr, context));
    ...
    const nextToken = () => {
        ...
        if (!_scnr.currentChar()) {
            return (_context.currentToken = getToken(_context, 15 /* EOF */));
        }
...
function createParser( /* options: ParserOptions = {} */) {
    ...
    */
    ...
    const parseMessage = (tokenizer) => {
        ...
        } while (context.currentType !== 15 /* EOF */ &&
...
function apply(app, composer, mode, ...options // eslint-disable-line @typescript-eslint/no-explicit-any
) {
    const pluginOptions = parseOptions(...options);
    if ((process.env.NODE_ENV !== 'production') && isString(pluginOptions['i18n-t'])) {
...
const generateCacheKey = (locale, key) => `${locale}__${key}`;
...
function defineMixin(legacyGlobal, composer) {
    return {
        beforeCreate() {
...
function useI18n(options = {}) {
    if (globalInstance === null)
        throw new Error('TODO'); // TODO:
    const globalComposer = '__composer' in globalInstance ? globalInstance.__composer : globalInstance;
...
const VERSION = "9.0.0-alpha.4";

export { Availabilities, VERSION, createCompiler, createI18n, useI18n };
