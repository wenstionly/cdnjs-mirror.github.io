...
function scrypt (key, salt, N, r, p, dkLen, callback) {
  ...
  var state = {i0: -1, r: r, N: N, B: B, V: V, XY: XY, state: 0};
  ...
  function incrementalSmix(state) {
      switch (state.state) {
          ...
          case 2:
              ...
              state.XY.copy(state.V, state.i1 * state.Yi, state.Xi, state.Xi + state.Yi)
          ...
          case 4:
              var key = state.XY.copy(state.B, state.Bi, state.Xi, state.Xi + state.Yi)
              callback(null, 1.0, pbkdf2Sync(key, B, 1, dkLen, 'sha256'))
              return;
      }
