(function (global, factory) {
	...
	DOM.append = function (node) {
		...
		for (var i$3 = 0, list$1 = nodes; i$3 < list$1.length; i$3 += 1) {
			...
			if (isInstance(i$1, EFFragment)) { i$1.appendTo(tempFragment); }
				...
				var mountingPoint = mountingPointStore.get(i$1);
	...
	var setDOMSimulation = function (sim) { return assign(DOM, sim); };

	if (isBrowser) { setDOMSimulation({Node: Node, document: document}); }
	...
	var getElement = function (ref$1) {
		...
		var element = createByTag({tagName: tagName, tagContent: tagContent, attrs: attrs, svg: svg});
	...
	var createElement = function (ref) {
		...
		var element = getElement({tagName: tagName, tagContent: tagContent, attrs: a, ref: r, refs: refs, svg: svg});
	...
	var resolveAST = function (ref) {
		...
		switch (nodeType) {
			...
			case 'array': {
				...
				if (typeOf(node[0]) === 'object') { DOM.append(element, create({node: node, ctx: ctx, innerData: innerData, refs: refs, handlers: handlers, subscribers: subscribers, svg: svg})); }
				// Dynamic text node
	...
	var create = function (ref) {
		...
		var element = createElement({info: info, ctx: ctx, innerData: innerData, refs: refs, handlers: handlers, subscribers: subscribers, svg: svg, fragment: fragment, custom: custom});
		if (fragment && 'development' !== 'production') { element.push(DOM.document.createComment('EF FRAGMENT START')); }
		...
		for (var i = 0, list = childNodes; i < list.length; i += 1) {
			...
			else { resolveAST({node: node$1, nodeType: typeOf(node$1), element: element, ctx: ctx, innerData: innerData, refs: refs, handlers: handlers, subscribers: subscribers, svg: svg, create: create}); }
		}
		if (fragment && 'development' !== 'production') { element.push(DOM.document.createComment('EF FRAGMENT END')); }
	...
	var EFBaseComponent = /*@__PURE__*/(function () {
		...
		if ( scope === void 0 ) scope = {};
			...
			nodeInfo.element = create({node: ast, ctx: ctx, innerData: innerData, refs: refs, handlers: handlers, subscribers: subscribers, svg: false});
	...
	var EFNodeWrapper = /*@__PURE__*/(function (EFBaseComponent) {
		function EFNodeWrapper() {
			...
			;(ref = (this).$ctx.nodeInfo.element).push.apply(ref, nodes);
	...
	exports.setDOMSimulation = setDOMSimulation;
