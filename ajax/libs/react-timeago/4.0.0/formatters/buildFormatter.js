...
var normalizeFn = function normalizeFn(value, numbers, distanceMillis) {
...
function buildFormatter(strings) {
  return function formatter(value, unit, suffix, epochMiliseconds) {
    var now = Date.now();
    // convert weeks to days if strings don't handle weeks
    if (unit === 'week' && !strings.week && !strings.weeks) {
      var _days = Math.round(Math.abs(epochMiliseconds - now) / (1000 * 60 * 60 * 24));
      value = _days;
      unit = 'day';
    }

    // create a normalize function for given value
    var normalize = normalizeFn(value, strings.numbers, now - epochMiliseconds);
    ...
    var wordSeparator = strings.wordSeparator == null ? ' ' : strings.wordSeparator;
